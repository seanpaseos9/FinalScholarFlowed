  ...
  try {
    ...
    if (snap.empty) {
      ...
      if (usersSnap.empty) {
        ...
        // Always ensure the default accounts exist and have correct password hashes.
        // This repairs any cases where accounts were accidentally deleted or hashes corrupted.
        await setDoc(doc(db, COLLECTIONS.USERS, INITIAL_STAFF_USER.id), INITIAL_STAFF_USER, { merge: true });
        await setDoc(doc(db, COLLECTIONS.USERS, INITIAL_ADMIN_USER.id), INITIAL_ADMIN_USER, { merge: true });
        console.log('Default staff/admin credentials refreshed.');
      }
    }
  } catch (error) {
...
export async function authenticateUserWithFirestore(
  ...
  try {
    ...
    // If Firestore returned nothing (offline, not seeded, or permissions issue),
    // fall back to the hardcoded default seeded accounts so login is never blocked.
    if (users.length === 0) {
      console.warn('Firestore users collection empty or unreachable — using default seeded accounts for authentication.');
      users = [INITIAL_STAFF_USER, INITIAL_ADMIN_USER];
    }
    ...
    // Even on total Firestore failure, try default credentials as last resort
    console.error('Firestore authentication error — attempting default credential fallback:', error);
    try {
      const fallbackUsers = [INITIAL_STAFF_USER, INITIAL_ADMIN_USER];
      const matchedUser = fallbackUsers.find(
        (u) => u.email && u.email.trim().toLowerCase() === cleanEmail
      );
      if (matchedUser && matchedUser.password) {
        const passwordMatches = await verifyPassword(pass, matchedUser.password);
        if (passwordMatches) {
          return { user: matchedUser, error: null };
        }
      }
    ...
    return {
      user: null,
      error: 'Could not connect to the database to verify account credentials. Please check your internet connection.',
    };
  }
}
